...
handle_info({'EXIT', Pid, Rsn}, S) ->
    ...
    case Rsn of
        normal -> error_logger:info_msg("~p done", [Name]);
        _      -> error_logger:error_msg("~p crashed: ~p", [Name, Rsn])
    end,
    {noreply, S#s{ p2n = dict:erase(Pid, S#s.p2n)
                  , n2p = dict:erase(Name, S#s.n2p)
                 }};
handle_info(Msg, S) ->
    error_logger:warning_msg("~p", [Msg]),
    {noreply, S}.
...
maybe_stop(Name, Options, P2N, N2P) ->
    case lists:keyfind(stop_on_remove, 1, Options) of
        false ->
            {P2N, N2P};
        _ ->
            case dict:find(Name, N2P) of
                {ok, Pid} ->
                    error_logger:info_msg("~p stopping", [Name]),
                    exit(Pid, removing),
                    {P2N, N2P};
                error ->
                    {P2N, N2P}
...
try_start(Name, Task, P2N, N2P) ->
    ...
    case dict:is_key(Name, N2P) of
        true ->
            error_logger:warning_msg("~p is running, not starting", [Name]),
            {P2N, N2P};
        false ->
            error_logger:info_msg("~p starting", [Name]),
            {M,F,A} = Task#task.mfa,
...
try_schedule(Name, Task, Tasks0, Queue0) ->
    case crontab_time:find_next(Task#task.spec, Task#task.next) of
        {ok, Time} ->
            error_logger:info_msg("~p next start: ~p", [Name, Time]),
            {gb_trees:update(Name, Task#task{next=Time}, Tasks0),
             gb_trees:insert({Time, Name}, Name, Queue0)};
        {error, Rsn} ->
            error_logger:info_msg("~p unable to find next start: ~p", [Name, Rsn]),
